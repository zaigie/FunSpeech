...
class AliyunBidirectionalTTSTestClient:
    ...
    async def test_bidirectional_synthesis(
        self, voice: str = "中文女", format: str = "PCM", sample_rate: int = 22050
    ):
        ...
        logger.info("\n" + "=" * 60)
        logger.info("交互式测试说明:")
        logger.info("1. 输入第一个文本片段")
        logger.info("2. 建立WebSocket连接并发送StartSynthesis")
        logger.info("3. 等待SynthesisStarted后立即发送第一个RunSynthesis")
        logger.info("4. 继续输入更多文本片段，按回车发送RunSynthesis")
        logger.info("5. 输入'stop'或'exit'发送StopSynthesis并结束")
        logger.info("=" * 60 + "\n")

        # 先获取第一个文本输入
        logger.info("🎯 请输入第一个文本片段:")
        first_text = await asyncio.get_event_loop().run_in_executor(
            None, lambda: input(f"[片段1] > ")
        )
        first_text = first_text.strip()

        if not first_text or first_text.lower() in ["stop", "exit", "quit", "q"]:
            logger.info("未输入有效文本，退出")
            return

        # 有了第一个文本后再建立WebSocket连接
        headers = {}
        if self.token:
        ...
        try:
            async with websockets.connect(
                ...
                # 3. 立即发送第一个RunSynthesis
                self.text_segments_count += 1
                await self._send_run_synthesis(first_text)

                # 4. 启动交互式文本输入（继续接收后续片段）
            ...
            try:
                ...
                if header.get("name") == "SynthesisStarted":
                    if header.get("status") == 20000000:
                        ...
                        raise Exception(
                            f"SynthesisStarted失败: {header.get('status_message')}"
                        )
    ...
    async def _interactive_text_input(self):
        """交互式文本输入处理（继续接收后续片段）"""
        logger.info("🎯 继续输入更多文本片段（输入'stop'结束）:")
                ...
                try:
                    ...
                    if header.get("name") == "SynthesisCompleted":
                        if header.get("status") == 20000000:
                            ...
                            raise Exception(
                                f"SynthesisCompleted失败: {header.get('status_message')}"
                            )
                    elif header.get("name") == "TaskFailed":
                        raise Exception(f"任务失败: {header.get('status_text')}")
                    else:
    ...
    async def _save_audio_file(self, format: str, sample_rate: int):
        ...
        if format.upper() == "PCM":
            ...
            # 转换PCM为WAV以便播放
            await self._convert_pcm_to_wav(
                self.audio_data, sample_rate, f"test_output_{timestamp}.wav"
            )
    ...
    async def _convert_pcm_to_wav(
        self, pcm_data: bytes, sample_rate: int, output_file: str
    ):
        """将PCM数据转换为WAV文件"""
        try:
            import struct
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="阿里云双向流式语音合成WebSocket测试工具"
    )
    parser.add_argument(
        "--url", default="ws://localhost:8000/ws/v1/tts", help="WebSocket服务URL"
    )
    parser.add_argument("--voice", default="中文女", help="音色名称")
    parser.add_argument(
        "--format", choices=["PCM", "WAV", "MP3"], default="PCM", help="音频格式"
    )
    parser.add_argument(
        "--sample-rate",
        type=int,
    ...
    try:
        ...
        await client.test_bidirectional_synthesis(
            voice=args.voice, format=args.format, sample_rate=args.sample_rate
        )
