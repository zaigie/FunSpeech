...
from ..models.websocket_tts import (
    AliyunWSMessage, AliyunWSHeader, AliyunStartSynthesisPayload, AliyunRunSynthesisPayload,
    AliyunSynthesisPayload, AliyunSubtitle,
    AliyunTTSNamespace, AliyunTTSMessageName, AliyunTTSStatus
)
from ..utils.common import generate_task_id, clean_text_for_tts, convert_speech_rate_to_speed
...
class AliyunWebSocketTTSService:
    ...
    def _initialize_engine(self):
        ...
        try:
            ...
            logger.info("阿里云WebSocket TTS引擎初始化成功")
        except Exception as e:
            logger.error(f"阿里云WebSocket TTS引擎初始化失败: {e}")
            raise e
        ...
        try:
            ...
            if hasattr(websocket, 'headers'):
                ...
                if settings.APPTOKEN and not x_nls_token:
                    await self._send_task_failed(websocket, task_id, "X-NLS-Token not found in ws header")
                    return
            ...
            while True:
                ...
                try:
                    ...
                    if namespace != AliyunTTSNamespace.FLOWING_SPEECH_SYNTHESIZER:
                        await self._send_task_failed(websocket, task_id, "Invalid namespace")
                        continue
                    ...
                    if message_name == AliyunTTSMessageName.START_SYNTHESIS:
                        if state == ConnectionState.READY:
                            # 验证和保存合成参数
                            synthesis_params = self._parse_start_synthesis(data, task_id)
                            if synthesis_params:
                                task_id = message_task_id or task_id
                                # 发送开始响应
                                await self._send_synthesis_started(websocket, task_id, session_id)
                                state = ConnectionState.STARTED
                            else:
                                await self._send_task_failed(websocket, task_id, "Invalid StartSynthesis parameters")
                        else:
                            await self._send_task_failed(websocket, task_id, "Connection already started")
                        ...
                        if state == ConnectionState.STARTED:
                            ...
                            await self._send_task_failed(websocket, task_id, "Connection not started")
                        ...
                        if state == ConnectionState.STARTED:
                            ...
                            # 完成合成
                            await self._send_synthesis_completed(websocket, task_id, session_id)
                        ...
                        await self._send_task_failed(websocket, task_id, f"Invalid message name: {message_name}")
                        
                except json.JSONDecodeError as e:
                    logger.error(f"[{task_id}] JSON解析错误: {e}")
                    await self._send_task_failed(websocket, task_id, f"Message Not Json: {message}")
                except Exception as e:
    ...
    async def _run_synthesis(self, websocket, task_id: str, session_id: str, text: str, params: dict):
        """执行流式合成"""
        ...
        try:
            ...
            async for audio_chunk in self._synthesize_streaming_audio(
                clean_text, local_voice, speed, params['format'], 
                params['sample_rate'], params['volume'], task_id
            ):
                if audio_chunk and len(audio_chunk) > 0:
                    ...
                    # 发送句子合成进度（可选）
                    await self._send_sentence_synthesis(websocket, task_id, session_id, text)
            ...
            await self._send_task_failed(websocket, task_id, f"Synthesis failed: {str(e)}")
    ...
    async def _synthesize_streaming_audio(
        ...
        format: str, sample_rate: int, volume: int, task_id: str
    ) -> AsyncGenerator[Optional[bytes], None]:
        """生成流式音频数据"""
        try:
            # 检查是否为克隆音色
            if self.tts_engine._voice_manager and self.tts_engine._voice_manager.is_voice_available(voice):
                if voice in self.tts_engine._voice_manager.list_clone_voices():
                    # 使用CosyVoice2流式合成（克隆音色）
                    async for chunk in self._stream_clone_voice(text, voice, speed, format, task_id):
                        yield chunk
                    return
            
            # 使用CosyVoice1流式合成（预设音色）
            if self.tts_engine.cosyvoice_sft:
                async for chunk in self._stream_preset_voice(text, voice, speed, format, task_id):
                    yield chunk
            else:
    ...
    async def _stream_preset_voice(
        ...
        for audio_data in self.tts_engine.cosyvoice_sft.inference_sft(
            ...
            if format.upper() == 'PCM':
                # PCM格式：直接发送16位PCM数据
                pcm_bytes = self._convert_audio_to_pcm(audio_array, self.tts_engine.cosyvoice_sft.sample_rate)
                yield pcm_bytes
            else:
                # WAV格式：转换为WAV字节流
                wav_bytes = self._convert_audio_to_wav(audio_array, self.tts_engine.cosyvoice_sft.sample_rate)
                yield wav_bytes
    ...
    async def _stream_clone_voice(
        ...
        """使用CosyVoice2进行流式合成（克隆音色）"""
        logger.info(f"[{task_id}] 使用CosyVoice2流式合成克隆音色: {voice}")
        
        # 使用CosyVoice2的stream=True功能进行零样本合成
        for audio_data in self.tts_engine.cosyvoice_clone.inference_zero_shot(
            text,
            ...
            if format.upper() == 'PCM':
                # PCM格式：直接发送16位PCM数据
                pcm_bytes = self._convert_audio_to_pcm(audio_array, self.tts_engine.cosyvoice_clone.sample_rate)
                yield pcm_bytes
            else:
                # WAV格式：转换为WAV字节流
                wav_bytes = self._convert_audio_to_wav(audio_array, self.tts_engine.cosyvoice_clone.sample_rate)
                yield wav_bytes
    ...
    async def _send_sentence_synthesis(self, websocket, task_id: str, session_id: str, text: str):
    ...
    async def _send_sentence_end(self, websocket, task_id: str, session_id: str, text: str):
        """发送SentenceEnd响应"""
        response = {
            "header": {
                "message_id": AliyunWSHeader.generate_message_id(),
