...
**关键约束**:
1. **qwen3-asr 不要同卡多副本** — vLLM 启动时直接预占 `QWEN3_ASR_GPU_MEM × 卡显存`,两个 qwen3-asr 副本同卡通常会 OOM。
2. **qwen3-tts-vllm-omni 基本按独占 24G 卡规划** — 4090 实测压测净占约 19-20 GiB; 和其它大模型共置前先把 `*_OMNI_GPU_MEM` 降下来并实测。
3. **legacy CosyVoice 可在显存够时同卡多副本** — 实测每增加一个同卡副本贡献约 `0.85 ×` 单副本容量,单客户端 RTF 会略升。`scripts/plan_deployment.py` 已按这个折扣估算。
4. **不同服务可以共置同一张卡** — 例如 funasr (3 GiB) + cosyvoice (5 GiB) 可以同卡, 共占 ~9 GiB; qwen3-asr 共置时要把 `QWEN3_ASR_GPU_MEM` 降到 0.3-0.4。
...
- TTS (cosyvoice / qwen3-tts-vllm-omni / cosyvoice3-vllm-omni): **想同时支持几路实时 TTS** (按 RTF ≤ 1 计)
- legacy `qwen3-tts`: 当前单路慢于实时,脚本会按隔离请求路数给出容量警告
...
| 单 24G 卡: qwen3-tts-vllm-omni 8 路实时 | 1× 4090 | 1 副本独占卡,约 19-20 GiB 净显存 |
...
> ⚠️ **同卡多副本要分服务看**: `qwen3-asr` 不要同卡多副本;`qwen3-tts-vllm-omni` 显存通常也只够独占。legacy CosyVoice 在显存够时可以同卡多副本,但按约 `0.85 ×` 折扣估算容量。手写 compose 容易算错,推荐直接用 `scripts/plan_deployment.py`。
...
# legacy qwen3-tts-0 写, qwen3-tts-1 / qwen3-tts-2 只读但能合成
QWEN3_TTS_SERVICE_URLS=http://qwen3-tts-0:8005,http://qwen3-tts-1:8005,http://qwen3-tts-2:8005

# qwen3-tts-vllm-omni-0 写,其它副本 reload 后可合成
QWEN3_TTS_VLLM_OMNI_SERVICE_URLS=http://qwen3-tts-vllm-omni-0:8006,http://qwen3-tts-vllm-omni-1:8006

# cosyvoice3-vllm-omni-0 写,其它副本 reload 后可合成
COSYVOICE3_VLLM_OMNI_SERVICE_URLS=http://cosyvoice3-vllm-omni-0:8007,http://cosyvoice3-vllm-omni-1:8007
```
...
CosyVoice、Qwen3-TTS 和两个 vLLM-Omni TTS facade 都暴露 `POST /voices/reload`。

- CosyVoice:用磁盘上的 `spk2info.pt` + `voice_registry.json` **覆盖**内存状态(磁盘没有的 zero-shot 音色会从内存里删掉, 但预设音色不动)。
- legacy Qwen3-TTS:重读 `voice_registry.json`,清空 prompt cache;只有 registry 中且 `prompts/{name}.pt` 存在的 clone voice 会对外可见。
- vLLM-Omni TTS:重读 `voice_registry.json`,并把目录里的 `*.wav` + `*.txt` 重新上传/注册到内层 vLLM-Omni Speech API。

| 场景 | 用法 |
|---|---|
| 网关写后自动广播 | 见 §5.2, 无需手动调用 |
| 直接更新文件后手动同步 | `curl -X POST -H "X-Internal-Token: $INTERNAL_SERVICE_TOKEN" http://cosyvoice-0:8004/voices/reload` 或对应 TTS 子服务的 `/voices/reload` |
| 排查不一致 | 任意副本调一次, 返回 `{clone_voices, registry_voices, clone_loaded}` |

CosyVoice 仅 `clone_loaded=true` 的副本(即 `TTS_MODEL_MODE` 包含 `clone`)才会真正重载 `spk2info`; `sft` only 副本只重载 registry。legacy Qwen3-TTS 与 Qwen3-TTS vLLM-Omni Base 当前按 Base Clone 语义接受音色 CRUD;Qwen3-TTS vLLM-Omni 切到 CustomVoice / VoiceDesign 时使用上游预置声线,不接受上传 clone。CosyVoice3 vLLM-Omni 是 clone only。
...
| `QWEN3_TTS_OMNI_DEFAULT_VOICE` | `vivian` | 未显式传 voice 时的默认声线;Base clone 场景仍建议传已注册 voice |
| `QWEN3_TTS_OMNI_LANGUAGE` | `Auto` | 透传 Speech API `language` |
...
| `COSYVOICE3_OMNI_FORCE_REF_PER_REQUEST` | `true` | 每次合成都把注册音色的 `ref_audio/ref_text` 发给 Speech API |
| `*_OMNI_SERVE_COMMAND` | `vllm` | 当前官方 CLI;只有镜像暴露其它命令时才改 |
| `*_OMNI_START_SERVER` | `true` | `false` 时 facade 不启动本地 vLLM,改为代理外部 `*_OMNI_API_BASE` |
| `*_OMNI_API_BASE` / `*_OMNI_API_KEY` | `http://127.0.0.1:8091` / `EMPTY` | 内层 vLLM-Omni Speech API 地址和鉴权 |
| `*_OMNI_PORT` | `8091` | 内层 vLLM-Omni 监听端口;不是 facade 对外端口 |
| `*_OMNI_STARTUP_TIMEOUT` | `900` | 等待 vLLM-Omni ready 的秒数 |
| `*_OMNI_REQUEST_TIMEOUT` | `300` | facade 调内层 Speech API 的请求超时 |
...
| `*_OMNI_TRUST_REMOTE_CODE` | `true` | 是否加 `--trust-remote-code` |
| `*_OMNI_ENFORCE_EAGER` | `false` | 是否加 `--enforce-eager` |
| `*_OMNI_NO_ASYNC_CHUNK` | `false` | 是否加 `--no-async-chunk` |

facade 对外端口仍用通用 `PORT`:Qwen3-TTS vLLM-Omni 默认 `8006`,CosyVoice3 vLLM-Omni 默认 `8007`。这两个后端不读取 legacy `TTS_LOAD_VLLM`;那个变量只属于 `services/cosyvoice` 的进程内 vLLM 开关。
...
- **多副本同 GPU 怎么算**: qwen3-asr 不要同卡;Qwen3-TTS vLLM-Omni 通常显存不够共置;legacy CosyVoice 可同卡但按约 `0.85 ×` 折扣估算。用规划脚本生成 compose,不要手算。
- **qwen3-asr 启动报 Hugging Face 离线缓存错误**: 离线场景务必把 `QWEN3_ASR_MODEL_ID` 设成本地路径,并设 `HF_HUB_OFFLINE=1` 和 `TRANSFORMERS_OFFLINE=1`, 见 §9.1。
- **切到非默认 TTS 后端但 cosyvoice 也起来了**: 不要只写 `--profile qwen3-tts-vllm-omni`;显式指定服务名,例如 `TTS_ENGINE=qwen3-tts-vllm-omni docker compose up -d gateway qwen3-asr-0 qwen3-tts-vllm-omni-0`。
- **vLLM-Omni TTS `/health` 长时间 `starting`**: 看对应子服务日志里内层 `vllm serve --omni` 是否在下载权重或等待编译;首次启动可接近 `start_period=600s`。离线场景确认 `${HF_CACHE}` 已包含对应模型。
- **CosyVoice3 vLLM-Omni 报 `voice not found`**: 该后端没有 `中文女` 预设音色,先向 `http://localhost:8007/voices` 上传 clone voice,或通过网关音色接口写到 primary 副本。
